...
const TEXT = {
  en: {
    ...
    "glossary.intro":
      "Names and terms the engine gets wrong without help - not every term you use. A phrase the engine already writes correctly only gives it something extra to guess with. Enter adds a phrase. The list is applied during recognition, not by editing finished text.",
    "glossary.placeholder": "Add a phrase…",
    "glossary.addAria": "Add a phrase to the glossary",
    "glossary.add": "Add",
    "glossary.remove": "Remove “{phrase}”",
    "glossary.weight": "weight: the engine only reads it as on (above zero) or off",
    "issue.weight_off": "Switched off by a weight of zero or less, and not passed to the engine at all: {list}.",
    "issue.weight_ignored": "The engine reads a weight as on or off, not as a strength - every positive value works the same: {list}.",
    "issue.weight_invalid": "The weight is not a number, so the engine falls back to the default: {list}.",
    "issue.comment": "The engine reads a line starting with # as a comment, so the phrase never reaches it: {list}.",
    "issue.tab": "A tab inside a phrase would split it in the engine's file, so it was replaced with a space: {list}.",
    "issue.short": "Very short phrases win close calls they were not meant to win: {list}.",
  ...
  ru: {
    ...
    "glossary.intro":
      "Имена и термины, которые движок без помощи пишет неверно, — а не все термины, которыми вы пользуетесь. Фраза, которую движок и так пишет правильно, только даёт ему лишний повод ошибиться. Фразу добавляет Enter. Список применяется на этапе распознавания, а не правкой готового текста.",
    ...
    "glossary.weight": "вес: движок читает его только как \"включено\" (больше нуля) или \"выключено\"",
    "issue.weight_off": "Выключены весом ноль или меньше и до движка не доходят вовсе: {list}.",
    "issue.weight_ignored": "Движок читает вес как \"включено\" или \"выключено\", а не как силу подсказки: любое положительное значение работает одинаково. Это {list}.",
    "issue.weight_invalid": "Вес не число, поэтому движок берёт значение по умолчанию: {list}.",
    "issue.comment": "Строку, начинающуюся с #, движок читает как комментарий, и фраза до него не доходит: {list}.",
    "issue.tab": "Табуляция внутри фразы разрезала бы её в файле движка, поэтому заменена пробелом: {list}.",
    "issue.short": "Очень короткие фразы выигрывают близкие совпадения там, где их не просили: {list}.",
